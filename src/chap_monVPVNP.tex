...
We have seen that monotone computational models are rather restrictive. We shall now see a  slight generalization of a lower bound by Jerrum and Snir~\cite{js82}. 

\section{Separating Monotone VP and VNP}\label{sec:monVPVNP}

We have seen exponential lower bounds against monotone circuits that use just sparsity as the complexity measure (with a clever monomial counting making use of the restrictive nature of computatation). 
Here, we shall see a beautiful result of Yehudayoff~\cite{Yeh18} that separates $\VP$ and $\VNP$ in the monotone world.
...
\begin{definition}[Monotone $\VNP$] A family of polynomial $\set{f_n(\vecx)} \in \R[\vecx]$ is said to be in monotone $\VNP$ (or $\monVNP$) if there is a family of polynomials $\set{g_n(\vecx, \vecy)} \in \monVP$ with $\abs{\vecy} = \poly{\abs{\vecx}}$ such that
  \[
    f(\vecx) = \sum_{\vecb \in \set{0,1}^{\abs{\vecy}}} g(\vecx, \vecb).
  \]
\end{definition}
...
The polynomial $P(\vecx)$ is set-multilinear with respect to rows, and any monotone model computing this must also be set-multilinear. Therefore, all intermediate computations must involve monomials that picks at most $1$ variable from each row.
