...
%   \quad \forall i\in [d]}$. 
Any linear combination of the above set is
...
%   $q^r$-sparse polynomials}. 
Hence, any depth-3 circuit $C$ over $\F$
...
matrix. 
For any $\mathcal{A}\subseteq \F^{n^2}$, let us define
...
product of at least $r-k$ independent linear polynomials. 
Let us count
...
The same bound shall also hold for $\Perm_n$. 
We shall defer this
...
(\autoref{def:entropy}). 
The only thing left to check is if we can
make $(2H_2(\gamma) - \beta \log q) > 0$ for some $\beta,\gamma$
satisfying $\beta > q^2 \gamma$ (as we needed $r > q^2 k$). 
It is not
hard to check that $\beta = q^{-q^2/2}$ suffices. 
Thus, we obtain the
bound
\begin{eqnarray*}
...
We now wish to show that $M_k(\Det_n;\mathcal{A})$ has large rank. 
The
...
entry to zero). 
Hence, $M_k(\Det_n)$ has the identity matrix
...
\Pr_{A}[p(A)\neq 0] & \geq & \Pr[x_{11} M_1' + \dots + x_{1n} M_n' + M'' \neq 0 \;|\; \text{some }M_{i}'\neq 0]\\
 & & \quad \times \Pr[\text{some }M_i' \neq 0]\\
  ...
  probability that $x_{11}M_1' + \dots + x_{1n}M_n'$ is non-zero! 
Hence,
\begin{eqnarray*}
...
We now give a few hints to prove \autoref{prop:random-det-nonzero}. 
