...
\begin{exercise}
  For any matrix $A$, we shall use the notation that $A[1|2]$ refers to the submatrix obtained by removing row column $1$ and row $2$.

  Show that, for a three-vertex \emph{glue gadget}, you need a $3\times 3$ matrix $A$ that satisfies the following three properties:
  \begin{itemize}
    \item $\mathrm{perm}(A) = \mathrm{perm}(A[1,3|2,3]) = 1$. 
    \item $\mathrm{perm}(A[3|3]) = \mathrm{perm}(A[1|2]) = \mathrm{perm}(A[3|1]) = \mathrm{perm}(A[3|2]) = 0$.
    \item $\mathrm{perm}(A[1,3|2,3]) = 1$. 
  \end{itemize}
