...
A very useful technique in arithmetic complexity is interpolation. Suppose we have a circuit $C$ that computes a polynomial $P(x_1,\cdots, x_n, y)$ and say $\deg_y P = d$. We can interpret the polynomial $P$ as an element of $(\F[\vecx])[y]$, that is we can think of $P$ as a univariate polynomial in $y$ with coefficients being elements of $\F[\vecx]$. 
\[
P(x_1,\cdots, x_n, y) \spaced{=} P_0(x_1,\cdots, x_n) + P_1(x_1,\cdots, x_n) y + \cdots + P_d(x_1,\cdots, x_n) y^d
\]
Given that we can compute $P$ by a circuit $C$, can we also compute one of the coefficients $P_i(x_1,\cdots, x_n)$ by a small circuit? The answer is `Yes' as long as we are working over a large enough field. 

\begin{lemma}[Interpolation]\label{lem:interpolation}
Let $P(x_1,\cdots, x_n, y)$ be a polynomial with $\deg_y P \leq d$. For any set of distinct scalars $\alpha_0, \cdots, \alpha_{d} \in \F$ and for every $i \in \set{0,\cdots, d}$,  each of the coefficient polynomials $P_i(x_1,\cdots, x_n)$ defined above can be expressed as a linear combination of 
\[
\set{P(x_1,\cdots, x_n, \alpha_0), \cdots, P(x_1,\cdots, x_n,\alpha_d)}.
\]
...
\end{lemma}
\begin{proof}
For this proof we shall use $P(\alpha_i)$ as a short-hand for $P(x_1,\cdots, x_n, \alpha_i)$. Then we have the following matrix identity
\[
...
\end{array}}
\]
In particular, for each $i \in \set{0,\cdots, d}$,
\[
P_i(x_1,\cdots, x_n) \spaced{=} \beta_0 P(x_1,\cdots, x_n, \alpha_0) + \cdots + \beta_d P(x_1,\cdots, x_n, \alpha_d). 
\]
...
Suppose $P(x_1,\cdots, x_n,y)$ be a polynomial computed by a size $s$ circuit with $\deg_y P \leq d$. Show that for any $i$, the partial derivative
...
\[
P(x_1,\cdots, x_n) \spaced{=} \Hom_0(P) + \cdots + \Hom_d(P)
\]
Let $y$ be a new variable and consider the polynomial $P'(x_1,\cdots, x_n, y) := P(yx_1, \cdots, y x_n)$. Then observe that
\[
P'(x_1,\cdots, x_n) \spaced{=} \Hom_0(P) + y\Hom_1(P) + \cdots + y^d \Hom_d(P).
\]
In other words, each homogeneous component of $P$ is a coefficient of some $y^i$ of $P'(x_1,\cdots, x_n,y)$. Thus, \autoref{lem:interpolation} tells us that we can use a linear combination of evaluations of $P'$ to compute these coefficients. 

We summarize this as a lemma. 

\begin{lemma}[Homogeneous component computations]\label{lem:computing-hom-components}
Let $P(x_1,\cdots, x_n)$ be a polynomial with $\deg P = d$. For any set of distinct scalars $\alpha_0, \cdots, \alpha_{d} \in \F$ and for every $i \in \set{0,\cdots, d}$,  each of the homogeneous components $\Hom_i(P)$ defined above can be expressed as a linear combination of 
\[
\set{P(\alpha_0x_1,\cdots, \alpha_0x_n), \cdots, P(\alpha_dx_1,\cdots, \alpha_dx_n)}.
\]
...
\[
\ESym_{d}(x_1,\cdots, x_n) \spaced{=} \sum_{1\leq i_1 < i_2 < \cdots < i_d \leq n} x_{i_1} \cdots x_{i_d}.
\]
\end{exercise}
...
\[
\sigma_d(x_1,\cdots, x_n) \spaced{=} \sum_{m \in \set{\text{deg. $d$ monomials}}} m.
\]
